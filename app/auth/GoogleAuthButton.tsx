...
export default function GoogleAuthButton({ returnTo, mode, disabled = false }: GoogleAuthButtonProps) {
  ...
  return (
    <>
      <button className={styles.googleButton} type="button" onClick={continueWithGoogle} disabled={disabled || pending}>
        {pending ? (
        ...
        <span>{pending ? "Duke u lidhur…" : mode === "sign-up" ? "Regjistrohu me Google" : "Vazhdo me Google"}</span>
      </button>
