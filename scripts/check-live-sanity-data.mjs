  ...
  *[_type == "grade" && isActive != false] | order(order asc) {
    ...
    "subjects": *[_type == "subject" && grade._ref == ^._id && isActive != false] {
      ...
      "chapters": *[_type == "chapter" && subject._ref == ^._id && isActive != false] {
        _id,
        "lessons": *[_type == "lesson" && chapter._ref == ^._id && isActive != false] { _id }
      }
    }
  }
`;
...
const counts = grades.reduce((total, grade) => {
  const subjects = Array.isArray(grade.subjects) ? grade.subjects : [];
  total.subjects += subjects.length;
  for (const subject of subjects) {
    const chapters = Array.isArray(subject.chapters) ? subject.chapters : [];
    total.chapters += chapters.length;
    for (const chapter of chapters) total.lessons += Array.isArray(chapter.lessons) ? chapter.lessons.length : 0;
  }
  return total;
