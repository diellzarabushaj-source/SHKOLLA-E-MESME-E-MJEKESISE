...
requireAll("Canonical Sanity write client", writeClient, [
  "SANITY_PROJECT_ID", "SANITY_DATASET", "SANITY_API_VERSION", "useCdn: false", "SANITY_API_WRITE_TOKEN",
]);
...
requireAll("Final portal pinning", pinner, [
  'projectId: "u5d5zn7n"', 'dataset: "schoolv2"', 'apiVersion: "2026-07-17"', "SANITY_PORTAL_DATA_INCOMPLETE",
]);
requireAll("Generated portal result", generatedPortal, [
  "sanity-v2-contract-v2", "canonical-sanity-schoolv2", 'projectId: "u5d5zn7n"', 'dataset: "schoolv2"',
  'apiVersion: "2026-07-17"', "freshClient.fetch<Lesson | null>", "SANITY_PORTAL_DATA_INCOMPLETE",
]);
if (generatedPortal.includes("projectId: process.env.NEXT_PUBLIC_SANITY_PROJECT_ID")) failures.push("Portali final nuk duhet të pranojë project ID nga environment.");
if (generatedPortal.includes("dataset: process.env.NEXT_PUBLIC_SANITY_DATASET_V2")) failures.push("Portali final nuk duhet të pranojë dataset nga environment.");
...
if (!String(packageJson.scripts?.["audit:app"] || "").includes("audit:admin-sanity")) failures.push("audit:app nuk e përfshin auditimin admin/Sanity.");
...
requireAll("Single administrator identity", adminIdentity, [
  ...
  'const ADMIN_PROVIDER = "google"', "user.emailVerified === true", "providers.length === 1", "await hasGoogleOnlyAccount(user.id)",
]);
requireAll("Admin rendering boundary", page, ["isCurrentUserAdmin", "isAdmin={isAdmin}", "isAuthenticated={Boolean(user?.id)}"]);
requireAll("Admin editor synchronization", adminEditor, [
  "readLatestFromSanity", 'cache: "no-store"', "Rifresko nga Sanity", "Ruaj në Sanity", "revision: currentLesson._rev", "onSaved(result.lesson)", "sanitizePastedHtml", "editorToPortable",
]);
requireAll("Admin loss prevention", adminHardening, ["beforeunload", "confirmLinkNavigation", "Të anulohen ndryshimet e paruajtura?", "INVALID_EMBEDDED_CONTENT"]);
requireAll("Admin API consistency", adminRoute, [
  "await requireAdminUser()", "isSameOriginRequest(request)", '{ perspective: "published" }', "current._rev !== revision", ".ifRevisionId(revision)", "sanitizeBody(payload.body", "const lesson = await readLesson(lessonId)", 'const noStoreHeaders = { "Cache-Control": "no-store" }',
]);

for (const [label, workflow] of [["Build workflow", buildWorkflow], ["Browser workflow", browserWorkflow]]) {
  requireAll(label, workflow, [`NEXT_PUBLIC_SANITY_PROJECT_ID: ${EXPECTED_PROJECT}`, `NEXT_PUBLIC_SANITY_DATASET_V2: ${EXPECTED_DATASET}`]);
}
